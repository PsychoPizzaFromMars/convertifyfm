...
export default function TimePeriodChart() {
  ...
  const handleSubmitPlaylist = (e) => {
    ...
    // TO DO:
    //     - reloading page after successful query
  };
  ...
  const { isLoaded, error, isSpotifyAuthenticated } = status;
  let results, scrollButtons;

  if (isSpotifyAuthenticated) {
    if (error) {
      results = (
        <div className="error-message">Error: {error.message || error}</div>
      );
    } else if (!isLoaded) {
  ...
  return (
    <>
      <div className="use-description">
        ...
        <p>
          Make Spotify playlist of your best music for different time periods.{" "}
        </p>
      </div>
